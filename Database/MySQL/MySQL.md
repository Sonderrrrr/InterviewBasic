...
- **B+ Tree 对比 二叉树**：对于有 N 个叶子节点的 B+ Tree，其搜索复杂度为 $O(log_dN)$，其中 d 表示节点允许的最大子节点个数。在实际的应用当中， d 值是大于 100 的，即使数据达到千万级别时，B+ Tree 的高度依然维持在 3~4 层左右，一次数据查询操作只需要做 3~4 次的硬盘 I/O 操作就能查询到。二叉树的每个父节点的儿子节点个数是 2 个，意味着其搜索复杂度为 $O(log_2N)$，二叉树检索到目标数据所经历的硬盘 I/O 次数要更多
