...
![TCP 的四次挥手](../Figures/Computer%20Network/TCP的四次挥手.svg)
...
客户端出现故障指客户端的主机发生了宕机或者断电。此时如果服务端一直不会发送数据给客户端，服务端永远无法感知到客户端宕机，即服务端的 TCP 连接将一直处于 `ESTABLISH` 状态，占用系统资源。因此 TCP 设置了保活机制监控连接的状态。
